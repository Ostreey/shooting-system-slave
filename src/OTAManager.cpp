...
OTAManager::OTAManager()
    : bleServer(nullptr), otaService(nullptr), otaCommandChar(nullptr),
      ...
      restartTime(0), isInitialized(false), otaBeginTaskHandle(nullptr),
...
{
    Serial.println("OTA: Error detected, scheduling reset in 2 seconds");
    pendingRestart = true;
    restartTime = millis() + 2000; // 2 seconds to allow error message to be sent
}
...
void OTAManager::handleStartCommand(const std::string &cmd)
{
    ...
    {
        Serial.println("OTA: Already in progress, ignoring START command");
        return;
    }
    ...
    Serial.println("OTA: Starting OTA begin task");
...
void OTAManager::otaBeginTask(void *pvParameters)
{
    ...
    Serial.println("OTA: OTA begin task started");

    // Step 3: Begin the update
    if (Update.begin(self->otaFirmwareSize))
    {
...
void OTAManager::processDataChunk(const std::string &chunk)
{
    if (!otaInProgress)
    {
        Serial.println("OTA: Data chunk received but OTA not in progress, ignoring");
        return;
    }
    ...
    {
        Serial.println("OTA: Data chunk received while OTA begin in progress, ignoring");
        return;
    }
